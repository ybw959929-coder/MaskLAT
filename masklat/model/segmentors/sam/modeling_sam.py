...
@dataclass
class SamMaskDecoderOutput(ModelOutput):
    """
    Base class for sam mask decoder model's outputs that also contains image embeddings obtained by applying the projection
    layer to the pooler_output.

    Args:
        iou_predictions (`torch.FloatTensor` of shape `(batch_size, num_masks)`):
...
class SamTwoWayTransformer(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            intermediate_hidden_states += (queries,)
        ...
        return queries, keys, all_attentions, intermediate_hidden_states
...
class SamMaskDecoder(nn.Module):
    def __init__(self, config: SamMaskDecoderConfig):
        ...
        self.query_tokens = nn.Embedding(self.num_queries, self.hidden_size)
    ...
    def forward(
        ...
        cond_lens: List[int] = None,
        output_attentions: Optional[bool] = None,
        return_dict: bool = True,
    ) -> Union[Tuple[torch.Tensor, torch.Tensor], SamMaskDecoderOutput]:
        """
        ...
        Args:
            ...
            cond_lens (List[int], *optional*):
                The lengths of the conditions.
        """
        # repeat the image embeddings according the cond_lens
        if cond_lens is not None:
            image_embeddings = torch.cat(
                [
                    image_embedding.unsqueeze(0).repeat(cond_len, 1, 1, 1)
                    for image_embedding, cond_len in zip(image_embeddings, cond_lens)
                ]
            )
            image_positional_embeddings = torch.cat(
                [
                    image_positional_embedding.unsqueeze(0).repeat(cond_len, 1, 1, 1)
                    for image_positional_embedding, cond_len in zip(image_positional_embeddings, cond_lens)
                ]
            )
        ...
        if sparse_prompt_embeddings.sum().item() != 0:
            ...
            point_batch_size = self.num_queries
            output_tokens = output_tokens.repeat(batch_size, point_batch_size, 1, 1)
            query_tokens = self.query_tokens.weight.repeat(batch_size, 1, 1).view(batch_size, self.num_queries, 1, -1)
            tokens = torch.cat((output_tokens, query_tokens), dim=2)
        ...
        # point_embeddings is the transformer layer outputs
        # image_embeddings is the last updated image_embeddings with transformer layers, attentions is the all attentions
        # intermediate_hidden_states are output of each transformer layers(2 layers in sam, actually is 3), which not include the last transformer layer
        if self.gradient_checkpointing and self.training:
            transformer_outputs = self._gradient_checkpointing_func(
                self.transformer.__call__,
                point_embeddings,
                image_embeddings,
        ...
        point_embeddings, image_embeddings, attentions, intermediate_hidden_states = transformer_outputs
        # upscaled_embeddings = ()
        ...
        # 1/32 feature
        # upscaled_embeddings += (image_embeddings,)

        # Upscale mask embeddings and predict masks using the mask tokens
        upscaled_embedding = self.upscale_conv1(image_embeddings)
        upscaled_embedding = self.activation(self.upscale_layer_norm(upscaled_embedding))
        # 1/16 feature
        # upscaled_embeddings += (upscaled_embedding,)

        upscaled_embedding = self.activation(self.upscale_conv2(upscaled_embedding))
        # 1/8 feature
        # upscaled_embeddings += (upscaled_embedding,)

        iou_token_out = point_embeddings[:, :, 0, :]
        mask_tokens_out = point_embeddings[:, :, 1 : (1 + self.num_mask_tokens), :]
        query_tokens_out = point_embeddings[:, :, -1, :]
        intermediate_hidden_states = tuple(
            [
                ...
                .reshape(batch_size, point_batch_size, self.hidden_size)
                .transpose(0, 1)
                for x in intermediate_hidden_states
            ]
        )
        ...
        masks = masks[:, :, mask_slice, :, :].squeeze(2)
        iou_pred = iou_pred[:, :, mask_slice].squeeze(-1)

        intermediate_hidden_states += (query_tokens_out.transpose(0, 1),)
        intermediate_mask_predictions += tuple([masks for _ in range(len(intermediate_hidden_states))])
        assert len(intermediate_hidden_states) == len(intermediate_mask_predictions)
        ...
        return SamMaskDecoderOutput(
            last_hidden_state=query_tokens_out.transpose(0, 1),
    ...
    def forward(
        ...
        Args:
            ...
            masks (`torch.Tensor`, *optional*):
                masks to embed
            input_embeds (`torch.Tensor`, *optional*):
        ...
        if input_embeds is not None:
            ...
            if sparse_embeddings is None:
                sparse_embeddings = input_embeds
            else:
                sparse_embeddings = torch.cat([sparse_embeddings, input_embeds], dim=2)
...
class SamVisionLayer(nn.Module):
    def __init__(self, config: SamVisionConfig, window_size):
        super().__init__()
