...
# Register project optimizer constructors before MMEngine/DeepSpeed builds the
# optimizer wrapper. Imports written only for side effects in lazy config files
# are not guaranteed to execute.
from masklat.engine.optim import CustomOptimWrapperConstructor  # noqa: F401, E402
...
def main():
    ...
    anomaly_enabled = os.environ.get(
        "MASKLAT_AUTOGRAD_ANOMALY",
        "0",
    ).strip().lower() in {"1", "true", "yes", "on"}
    if anomaly_enabled:
        torch.autograd.set_detect_anomaly(True)
        print_log(
            "Autograd anomaly detection is enabled for non-finite backward "
            "diagnosis.",
            logger="current",
